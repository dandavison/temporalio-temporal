...
func (s *activityParityTestSuite) TestConformance() {
	// The budget is established three times over: here for the envs, which are created against s.T(),
	// and again in each explorer for the drivers, which anchor on their own subtest.
	...
	s.T().Run("WFARandomWalk", s.conformanceWFARandomWalk)
}
...
// saaConformanceContextBudget is one explorer's context deadline: the larger of DefaultTimeout (which
// reflects TEMPORAL_TEST_TIMEOUT) and a floor that fits that explorer at its default depth. Each
// explorer establishes it on its own subtest, because that is the testing.TB its drivers anchor on;
// setting it on the parent leaves the subtests at the 90s default. A run past the default depth needs
// TEMPORAL_TEST_TIMEOUT raised beyond the floor, alongside `go test -timeout`.
