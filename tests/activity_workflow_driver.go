...
func (a *wfaHandle) awaitDispatchDelay(t require.TestingT, e model.Event) {
	awaitActivityDispatchDelay(t, e, func() activityDispatchDelayObservation {
		pa := a.pendingActivity(t)
		if pa == nil {
			return activityDispatchDelayObservation{
				state:   activityDispatchUnavailable,
				summary: "activity is no longer pending",
			}
		}
		observation := activityDispatchDelayObservation{summary: fmt.Sprintf("%+v", wfaActivityInfo(pa))}
		switch pa.GetState() {
		case enumspb.PENDING_ACTIVITY_STATE_SCHEDULED:
			if next := pa.GetNextAttemptScheduleTime(); next != nil {
				observation.state = activityDispatchPending
				observation.dueTime = next.AsTime()
			} else {
				observation.state = activityDispatchDue
			}
		case enumspb.PENDING_ACTIVITY_STATE_STARTED:
			observation.state = activityDispatchStarted
		default:
			observation.state = activityDispatchUnavailable
		}
		return observation
	})
}
