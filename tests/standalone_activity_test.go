...
func (s *standaloneActivityTestSuite) TestUnpauseActivityExecution() {
	...
	// UnpauseWhileCancelRequested: pause-then-cancel transitions PAUSE_REQUESTED → CANCEL_REQUESTED;
	// the cancel supersedes the pause (no zombie pause-state survives on a cancel-requested
	// activity). Issuing Unpause on a CANCEL_REQUESTED activity is therefore a no-op.
	t.Run("UnpauseWhileCancelRequested", func(t *testing.T) {
		...
		// Cancel → CANCEL_REQUESTED. The cancel transition supersedes the pause.
		_, err = env.FrontendClient().RequestCancelActivityExecution(ctx, &workflowservice.RequestCancelActivityExecutionRequest{
			Namespace:  env.Namespace().String(),
			ActivityId: activityID,
			RunId:      runID,
		...
		require.False(t, hbResp.GetActivityPaused(), "cancel supersedes pause; ActivityPaused must be false")
