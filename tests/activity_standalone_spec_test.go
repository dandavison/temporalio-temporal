...
// every step. The wall-clock scenarios (timeouts, dispatch delays) are checked by TestActivityTraces
// (in activity_standalone_test.go); the exhaustive RPC-graph and random-walk explorers are here. The
// engine that drives and checks each event is in activity_standalone_spec_harness.go, built on the
// driver in activity_standalone_utils.go.
