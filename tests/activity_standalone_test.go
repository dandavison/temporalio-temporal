...
func (s *standaloneActivityTestSuite) TestResetActivityExecution() {
	env := s.newTestEnv()
	t := s.T()
	initialStartToCloseTimeout := durationpb.New(15 * time.Minute)
	...
	startAndPollActivity := func(ctx context.Context, t *testing.T, activityID string, retryPolicy *commonpb.RetryPolicy) (
		...
		*workflowservice.PollActivityTaskQueueResponse,
		...
		startResp, err := env.FrontendClient().StartActivityExecution(ctx, &workflowservice.StartActivityExecutionRequest{
			...
			Input:               defaultInput,
			TaskQueue:           &taskqueuepb.TaskQueue{Name: taskQueue},
			StartToCloseTimeout: initialStartToCloseTimeout,
	...
	t.Run("WhileRunning_AttemptFailsRetryably", func(t *testing.T) {
		...
		// Fail the running attempt retryably — triggers deferred reset in TransitionRescheduled
	...
	// Reset(RestoreOriginal) is applied while an attempt is running, then the attempt fails
	// retryably. Assert that the reset is honored and the next attempt has the restored values. See
	// also ResetRestoreOriginal_OnStarted_DefersPerAttemptOptionRestore (in TestStartDelay).
	t.Run("ResetRestoreOriginal_WhileRunning_AttemptFailsRetryably", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
