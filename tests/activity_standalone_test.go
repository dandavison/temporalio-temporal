...
// TestDescribeNextAttemptScheduleTimeAndCurrentRetryInterval
...
func (s *standaloneActivityTestSuite) TestDescribeNextAttemptScheduleTimeAndCurrentRetryInterval() {
	...
	t.Run("FirstAttemptRunning", func(t *testing.T) {
		info := s.driveTrace(t, env, saaTrace{trace: []model.Event{saaPoll}, maxAttempts: 3, retryInterval: saaDelayWindow}).describe(t).GetInfo()
	...
	t.Run("BackingOffBeforeRetry", func(t *testing.T) {
		info := s.driveTrace(t, env, saaTrace{trace: []model.Event{saaPoll, saaFailRetryably}, maxAttempts: 3, retryInterval: saaDelayWindow}).describe(t).GetInfo()
