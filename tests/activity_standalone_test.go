...
func (s *standaloneActivityTestSuite) TestStartDelay() {
	...
	// activity is paused in the delay window: UpdateActivityOptions can change anything while paused.
	// The update must be accepted (not rejected as "outside the delay window") and the new value takes
	// effect on unpause.
	s.Run("UpdateWhilePaused_ShortensDispatch", func(s *standaloneActivityTestSuite) {
		...
		activityID := testcore.RandomizeStr(t.Name())
		taskQueue := testcore.RandomizeStr(t.Name())
		// originalDelay >> the 10s poll context: if the paused update didn't take effect, the poll would
		// time out. newDelay is short so the poll returns quickly under the updated delay.
		originalDelay := 60 * time.Second
		newDelay := 2 * time.Second
		...
		// Pause while inside the delay window; with no attempt dispatched yet the activity goes straight
		// to PAUSED.
		_, err = env.FrontendClient().PauseActivityExecution(s.Context(), &workflowservice.PauseActivityExecutionRequest{
			Namespace:  env.Namespace().String(),
			ActivityId: activityID,
		...
		// Update start_delay while paused: must be accepted, not rejected as outside the delay window.
		_, err = env.FrontendClient().UpdateActivityExecutionOptions(s.Context(), &workflowservice.UpdateActivityExecutionOptionsRequest{
			Namespace:       env.Namespace().String(),
			ActivityId:      activityID,
			RunId:           startResp.RunId,
