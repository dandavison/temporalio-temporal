...
func (a *saaActor) apply(t require.TestingT, e model.Event, cur model.AbstractState, out model.Outcome, final bool) saaApply {
...
func (a *saaActor) verify(t require.TestingT, e model.Event, cur model.AbstractState, out model.Outcome, rpcErr error, final bool) bool {
...
func (a *saaActor) checkTaskInvalidation(t require.TestingT, e model.Event, cur model.AbstractState, out model.Outcome) {
...
func (a *saaActor) applyPoll(cur model.AbstractState, out model.Outcome, final bool, t require.TestingT) saaApply {
...
func (a *saaActor) applyWallClock(t require.TestingT, e model.Event, cur model.AbstractState, out model.Outcome, final bool) saaApply {
...
func (h *saaHarness) driveTrace(t *testing.T, trace []model.Event) {
	a := h.start(t)
	a.path = trace
	cur := model.Initial(h.cfg)
...
// ExpectedDescribe.
func (a *saaActor) checkDescribe(t require.TestingT, expected model.AbstractState) {
...
func (h *saaHarness) walkStart(t *testing.T) (*saaActor, model.AbstractState) {
	a := h.start(t)
	cur := model.Initial(h.cfg)
	obs, err := a.observed()
	require.NoError(t, err)
...
func (h *saaHarness) pickWalkEvent(rng *rand.Rand, a *saaActor, cur model.AbstractState) model.Event {
...
// Started".
func (a *saaActor) edge(e model.Event, src model.Status) string {
...
func (a *saaActor) stateFailure(e model.Event, src model.Status, observed, expected model.AbstractState) string {
	var summary string
...
func (a *saaActor) rejectFailure(e model.Event, src model.Status, got, want model.ErrorKind, err error) string {
...
func (a *saaActor) flagsFailure(e model.Event, src model.Status, observed, expected model.HeartbeatFlags) string {
