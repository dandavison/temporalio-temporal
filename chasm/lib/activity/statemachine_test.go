...
// A terminal timeout must chain the failure that drove the retries as its Cause, so an SDK can
// surface the real failure rather than the timeout that happened to close the activity. See
// mutable_state_impl.go AddActivityTaskTimedOutEvent and temporalio/temporal#3667.
func TestTransitionTimedOutChainsPriorAttemptFailure(t *testing.T) {
	testCases := []struct {
		name string
		// timeoutType is the timeout that fired.
		timeoutType enumspb.TimeoutType
		// retryState is what shouldRetry concluded when the timeout fired.
		retryState enumspb.RetryState
		// expectedTimeoutType is the timeout the closed activity reports. A timeout that leaves too
		// little of the schedule-to-close deadline for another attempt is reported as a
		// schedule-to-close timeout, since it is the deadline, not the attempt, that closed it.
...
// A timeout that is retried must not chain the previous attempt's failure: each attempt's failure
// replaces the last one, so chaining here would nest one more level per attempt and grow the stored
// failure without bound. Only the terminal timeout chains, and it chains one level.
