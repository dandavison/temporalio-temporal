...
func (a *Activity) handleReset(ctx chasm.MutableContext, req *activitypb.ResetActivityExecutionRequest) (*activitypb.ResetActivityExecutionResponse, error) {
	...
	case activitypb.ACTIVITY_EXECUTION_STATUS_PAUSED:
		...
		if keepPaused {
			...
			// later unpause honors any remaining backoff (see honoredDispatchTime).
			attempt := a.LastAttempt.Get(ctx)
			attempt.Count = 1
			attempt.Stamp++
			if frontendReq.GetResetHeartbeat() {
...
// the first attempt, via dispatchTimeRespectingStartDelay) and a pending retry backoff (for retries,
// via the attempt's CompleteTime + CurrentRetryInterval). base is the earliest acceptable time (now,
// possibly offset by jitter). This mirrors reissueDispatchAndScheduleToStart, so every re-dispatch
// path treats a pending backoff the same way.
