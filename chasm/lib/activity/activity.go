...
// isPaused reports whether the activity is currently paused (waiting) or has pending pause
// (worker still running). PauseState alone is audit data and outlives the paused condition; the
...
// recordPauseState stamps audit data (identity, reason, time, request_id) onto the activity.
// Behavior — whether dispatch is blocked, whether the worker is told to pause, etc. — is driven by
// Status, not by PauseState. The request_id field is consulted by handlePauseRequested for
// idempotency.
...
func (a *Activity) handleReset(ctx chasm.MutableContext, req *activitypb.ResetActivityExecutionRequest) (*activitypb.ResetActivityExecutionResponse, error) {
	...
	case activitypb.ACTIVITY_EXECUTION_STATUS_STARTED,
		...
		activitypb.ACTIVITY_EXECUTION_STATUS_PAUSE_REQUESTED:
		// Worker is still running. Defer reset to the next retry so we don't break
		...
		if !keepPaused && a.Status == activitypb.ACTIVITY_EXECUTION_STATUS_PAUSE_REQUESTED {
			// Unpause the worker; the deferred reset will apply on the next retry via the
			// regular STARTED→SCHEDULED path rather than the PAUSE_REQUESTED→PAUSED path.
	...
	case activitypb.ACTIVITY_EXECUTION_STATUS_PAUSED:
		if keepPaused {
			// Reset counts but keep the activity paused. No dispatch task — user must unpause.
			attempt := a.LastAttempt.Get(ctx)
			attempt.Count = 1
			attempt.Stamp++
			attempt.CurrentRetryInterval = nil
...
// if retry is not possible. From STARTED the retry lands in SCHEDULED with a fresh dispatch task.
// From PAUSE_REQUESTED the retry lands in PAUSED — no dispatch task is emitted; the activity
