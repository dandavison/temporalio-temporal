...
func (a *Activity) HandleUpdateOptions(
	ctx chasm.MutableContext,
	...
	switch a.GetStatus() {
	case activitypb.ACTIVITY_EXECUTION_STATUS_SCHEDULED, activitypb.ACTIVITY_EXECUTION_STATUS_PAUSED:
...
func (a *Activity) handleReset(ctx chasm.MutableContext, req *activitypb.ResetActivityExecutionRequest) (*activitypb.ResetActivityExecutionResponse, error) {
	...
	case activitypb.ACTIVITY_EXECUTION_STATUS_PAUSED:
		if keepPaused {
			// Reset counts but keep the activity paused. The pending retry backoff is preserved so
			// that a later unpause honors any remaining backoff (see honoredDispatchTime).
			attempt := a.LastAttempt.Get(ctx)
			attempt.Count = 1
			attempt.Stamp++
			if frontendReq.GetResetHeartbeat() {
...
// honoredDispatchTime returns the time at which a re-dispatch (unpause/reset) should occur, never
// earlier than the activity's already-scheduled dispatch time: it honors a pending start_delay (for
// the first attempt, via clampToDispatchTime) and a pending retry backoff (for retries, via the
// attempt's CompleteTime+CurrentRetryInterval). base is the earliest acceptable time (now, possibly
// offset by jitter). This mirrors reissueDispatchAndScheduleToStart, which the options-update path
// already uses, so all re-dispatch paths treat a pending backoff the same way.
func (a *Activity) honoredDispatchTime(attempt *activitypb.ActivityAttemptState, base time.Time) time.Time {
	dispatchTime := a.clampToDispatchTime(base)
	if retryTime := attemptDispatchTimeForRetry(attempt); retryTime != nil && retryTime.AsTime().After(dispatchTime) {
