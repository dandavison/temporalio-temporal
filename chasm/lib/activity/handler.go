...
// GetActivityExecutionOutcome long-polls for activity outcome. It returns an empty non-error
// response on context deadline expiry, to indicate that the state being waited for was not reached.
// Callers should interpret this as an invitation to resubmit their long-poll request. This response
// is sent before the caller's deadline (see chasm.activity.longPollBuffer) so that it is likely
// that the caller does indeed receive the non-error response.
