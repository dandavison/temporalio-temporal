...
//
// The two stamps are not part of AbstractState: their only observable meaning is that a pending task
// was invalidated, i.e. the value changed. So the model states, per transition, whether each bumps,
// and the harness compares the observed stamp delta across the edge against these booleans.
type Outcome struct {
	Next                       AbstractState
	Reject                     ErrorKind
	StampBumped                bool // this transition invalidates the pending attempt dispatch/timer tasks
	ScheduleToCloseStampBumped bool // this transition restarts/invalidates the schedule-to-close timer
}
