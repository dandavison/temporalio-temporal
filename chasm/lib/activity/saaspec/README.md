...
Configure one timeout short, wait past it, and check the resulting state against `Model` for that
`(state, timer-event)`. The outcome is never hard-coded — it comes from `Model`.
