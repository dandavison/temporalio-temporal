...
// Marshal an object of type PendingReset to the protobuf v3 wire format
func (val *PendingReset) Marshal() ([]byte, error) {
	return proto.Marshal(val)
}

// Unmarshal an object of type PendingReset from the protobuf v3 wire format
func (val *PendingReset) Unmarshal(buf []byte) error {
	return proto.Unmarshal(buf, val)
}

// Size returns the size of the object, in bytes, once serialized
func (val *PendingReset) Size() int {
	return proto.Size(val)
}

// Equal returns whether two PendingReset values are equivalent by recursively
// comparing the message's fields.
// For more information see the documentation for
// https://pkg.go.dev/google.golang.org/protobuf/proto#Equal
func (this *PendingReset) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	var that1 *PendingReset
	switch t := that.(type) {
	case *PendingReset:
		that1 = t
	case PendingReset:
