...
// PAUSE_REQUESTED is allowed when the operator issues reset with keepPaused=true: pending_reset.keep_paused
// is set so the activity lands back in PAUSED (not SCHEDULED) when the worker yields. The worker is
...
// when the worker yields in RESET_REQUESTED with pending_reset.keep_paused set (i.e. reset was issued
// with keepPaused=true while the activity was in PAUSE_REQUESTED). The failed attempt is recorded, the
